...
if HAS_PSYCOPG:
    ...
    try:
        ...
        cur.execute("SELECT event_type, count(*) FROM normalized_events GROUP BY event_type ORDER BY count(*) DESC LIMIT 10")
        print("  By type:")
        for r in cur.fetchall():
            print(f"    {r[0]}: {r[1]}")
        
        cur.execute("SELECT timestamp, event_type, source_ip, dest_ip, dest_port, severity FROM normalized_events ORDER BY timestamp DESC LIMIT 5")
        print("  Recent events:")
        for r in cur.fetchall():
            print(f"    {r[0]} | {r[1]:15s} | {r[2]:15s} -> {r[3]:15s}:{r[4]} | {r[5]}")
        
        cur.execute("SELECT source_ip, count(*) FROM normalized_events GROUP BY source_ip ORDER BY count(*) DESC LIMIT 10")
        print("  Top sources:")
        for r in cur.fetchall():
            print(f"    {r[0]}: {r[1]}")
        
        cur.execute("SELECT action, count(*) FROM normalized_events GROUP BY action ORDER BY count(*) DESC")
        print("  Actions:")
        for r in cur.fetchall():
            print(f"    {r[0]}: {r[1]}")
        
        cur.execute("SELECT timestamp, count(*) FROM normalized_events GROUP BY date_trunc('hour', timestamp) ORDER BY 1 DESC LIMIT 5")
