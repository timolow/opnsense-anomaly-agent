...
class DiscordClient:
    ...
    DEDUP_SECONDS = 300  # Don't re-alert on the same IP within 5 min

    def send_incident_alert(self, incident_dict: Dict[str, Any]) -> bool:
        """Send a unified incident alert to Discord.

        Produces a rich embed with IP + DNS name, behavioral score, threat level,
        signal summary, attack chain, narrative, and recommended actions.

        Deduplicates per-IP within DEDUP_SECONDS.

        Args:
            incident_dict: Incident.to_dict() output with 'ip', 'severity',
                'narrative', 'signal_types', 'phases', 'sources', etc.

        Returns:
            True if sent successfully, False otherwise.
        """
        ip = incident_dict.get('ip', 'unknown')
        severity = incident_dict.get('severity', 'low')
        narrative = incident_dict.get('narrative', '')
        signal_count = incident_dict.get('signal_count', 0)
        sources = incident_dict.get('sources', [])
        signal_types = incident_dict.get('signal_types', [])
        phases = incident_dict.get('phases', [])
        is_escalated = incident_dict.get('is_escalated', False)
        affected_targets = incident_dict.get('affected_targets', [])
        related_ips = incident_dict.get('related_ips', [])
        ...
        if not self.rate_limiter.should_alert(dedup_key, dedup_key):
            logger.debug("Deduplicating incident alert for %s (within %ds)", ip, self.DEDUP_SECONDS)
            return False

        # ── Title ──
        if is_escalated:
            title = f"🚨 ESCALATED INCIDENT: {ip}"
        elif len(phases) >= 3:
            title = f"⚠️ ATTACK CHAIN: {ip}"
        elif len(phases) >= 2:
            title = f"🔶 MULTI-PHASE INCIDENT: {ip}"
        else:
            title = f"🔍 INCIDENT: {ip}"

        # Append DNS name to title if available
        if dns_name:
            title = title.replace(f": {ip}", f": {ip} ({dns_name})")

        # ── Recommended actions ──
        actions = self._recommend_actions(severity, phases, signal_types, is_escalated)

        # ── Signal summary ──
        source_summary = ', '.join(sources) if sources else 'N/A'
        signal_summary = ', '.join(signal_types[:6])
        if len(signal_types) > 6:
            signal_summary += f' (+{len(signal_types) - 6} more)'

        # ── Build fields ──
        fields: List[Dict[str, Any]] = []

        # Row 1: Severity + Signals + Sources
        fields.append({'name': 'Severity', 'value': severity.upper(), 'inline': True})
        fields.append({'name': 'Signals', 'value': f'{signal_count} signals', 'inline': True})
        fields.append({'name': 'Sources', 'value': source_summary[:250], 'inline': True})

        # Behavioral score (if available)
        if behavioral_score is not None:
            score_display = f"{behavioral_score:.1f}/1.0"
            score_indicator = '🔴' if behavioral_score > 0.7 else '🟠' if behavioral_score > 0.4 else '🟡' if behavioral_score > 0.2 else '🔵'
            fields.append({'name': f'Behavioral Score {score_indicator}', 'value': score_display, 'inline': True})
            fields.append({'name': 'Threat Level', 'value': self._score_to_threat(behavioral_score), 'inline': True})
            fields.append({'name': '', 'value': ' ', 'inline': True})  # Spacer

        # Attack chain (if multi-phase)
        if phases and len(phases) >= 2:
            chain_str = ' → '.join(phases)
            fields.append({'name': 'Attack Chain', 'value': chain_str, 'inline': False})

        # Chain timeline (compact)
        if chain_timeline and len(chain_timeline) >= 2:
            timeline_parts = []
            for entry in chain_timeline:
                ts = entry.get('timestamp', 0)
                dt = datetime.fromtimestamp(ts, tz=timezone.utc)
                timeline_parts.append(f"{dt.strftime('%H:%M')} {entry['phase']} ({entry['signal_type']})")
            fields.append({'name': 'Timeline', 'value': '\n'.join(timeline_parts[:8]), 'inline': False})

        # Signal types
        if signal_summary:
            fields.append({'name': 'Signal Types', 'value': signal_summary[:500], 'inline': False})

        # Affected targets
        if affected_targets:
            targets_str = ', '.join(str(t) for t in affected_targets[:10])
            if len(affected_targets) > 10:
                targets_str += f' (+{len(affected_targets) - 10} more)'
            fields.append({'name': 'Affected Targets', 'value': targets_str[:500], 'inline': False})

        # Related IPs (cross-source correlation)
        if related_ips and len(related_ips) > 1:
            fields.append({'name': 'Related IPs', 'value': ', '.join(sorted(related_ips)), 'inline': False})
        ...
        # Truncate description to leave room for fields (Discord embed limit: 6000 chars total)
        total_field_chars = sum(len(f.get('name', '')) + len(f.get('value', '')) for f in fields)
        max_desc = 6000 - total_field_chars - 500  # reserve buffer
        ...
        result = self._post(f'channels/{self.channel_id}/messages', payload)
        if result:
            logger.info("Unified incident alert sent for %s [%s] (score=%s)", ip, severity.upper(), behavioral_score or 'N/A')
        return result

    def _recommend_actions(self, severity: str, phases: List[str], signal_types: List[str], is_escalated: bool) -> List[str]:
        """Generate recommended actions based on incident characteristics."""
        actions = []

        if is_escalated or severity == 'critical':
            actions.append('🔴 IMMEDIATE: Block source IP at firewall')
            actions.append('🔴 Review IDS/IPS logs for exploit attempts')
        elif severity == 'high':
            actions.append('🟠 Investigate source IP — check for known threat intel')
            actions.append('🟠 Verify firewall rules are blocking malicious traffic')
        elif severity == 'medium':
            actions.append('🟡 Monitor source IP for escalation')
            actions.append('🟡 Check correlation dashboard for related incidents')

        # Phase-specific recommendations
        if 'exploit' in phases:
            actions.append('💻 Check compromised hosts for lateral movement')
        if 'attack' in phases:
            actions.append('🛡️ Verify WAF/IPS signatures are up to date')
        if 'recon' in phases and 'probe' not in phases:
            actions.append('👁 Recon activity — consider IP reputation lookup')

        return actions

    def _score_to_threat(self, score: float) -> str:
        """Map behavioral score to threat level label."""
        if score > 0.8:
            return 'CRITICAL'
        if score > 0.6:
            return 'HIGH'
...
class CommandHandler:
    ...
    def _cmd_incident(self, args: str) -> CommandResult:
        """Handle /incident <inc_id|ip> [transition|feedback] [status|feedback_type] — View or act on an incident."""
        parts = args.strip().split()
        if not parts:
            return CommandResult(content="Usage: `/incident <inc_id|ip> [action] [value]`\n\nActions:\n  (none) — View incident details (accepts inc_xxxxxxxx ID or IP address)\n  `transition <status>` — Transition to: investigating, confirmed, resolved\n  `feedback thumbs_up|thumbs_down` — Record feedback\n  `stats` — Show incident statistics")
        ...
        # Resolve identifier: try as inc_id first, then as IP
        inc_id = None
        if identifier.startswith('inc_'):
            inc_id = identifier
        else:
            # Check if it looks like an IP address
            if self._looks_like_ip(identifier):
                # Look up incident by IP from correlation engine
                corr = getattr(self.agent, 'correlation_engine', None)
                if corr:
                    incident = corr.get_incident_by_ip(identifier)
                    if incident:
                        # Find matching IncidentRecord in manager
                        inc_id = self._find_inc_id_by_ip(mgr, identifier)
                    if not inc_id:
                        return CommandResult(content=f"No active incident found for IP `{identifier}`.")
                else:
                    return CommandResult(content="Correlation engine not available for IP lookup.")
            else:
                return CommandResult(content=f"Invalid identifier: `{identifier}`. Provide an incident ID (inc_xxxxxxxx) or IP address.")
        ...
        if action == "transition":
            ...
            # Enrich with correlation engine data for unified view
            corr = getattr(self.agent, 'correlation_engine', None)
            corr_incident = None
            if corr:
                corr_incident = corr.get_incident_by_ip(result['ip'])

            # Behavioral score
            behavioral_score = None
            behavior_profiler = getattr(self.agent, 'behavior_profiler', None)
            if behavior_profiler:
                try:
                    behavioral_score = behavior_profiler.get_behavioral_score(result['ip'])
                except Exception:
                    pass

            # DNS name
            dns_name = None
            reverse_dns = getattr(self.agent, 'reverse_dns', None)
            if reverse_dns and getattr(reverse_dns, 'enabled', False):
                try:
                    dns_name = reverse_dns.lookup(result['ip'])
                except Exception:
                    pass
            ...
            if dns_name:
                ip_display = f"{result['ip']} ({dns_name})"
            ...
            if behavioral_score is not None:
                score_indicator = '🔴' if behavioral_score > 0.7 else '🟠' if behavioral_score > 0.4 else '🟡' if behavioral_score > 0.2 else '🔵'
                fields.append({"name": f"Behavioral Score {score_indicator}", "value": f"{behavioral_score:.1f}/1.0", "inline": True})

            # Attack chain from correlation engine
            if corr_incident:
                chain = corr_incident.get_attack_chain()
                if len(chain) >= 2:
                    fields.append({"name": "Attack Chain", "value": " → ".join(chain), "inline": False})
                narrative = corr_incident.get_narrative(reverse_dns if reverse_dns and getattr(reverse_dns, 'enabled', False) else None)
                if narrative:
                    fields.append({"name": "Narrative", "value": narrative[:1024], "inline": False})
            ...
            return CommandResult(content=f"**{inc_id}** — {ip_display}", embed=embed)
    ...
    def _find_inc_id_by_ip(self, mgr, ip: str) -> Optional[str]:
        """Find an active incident ID in the manager for a given IP."""
        ip_incs = getattr(mgr, '_ip_incidents', {}).get(ip, [])
        for inc_id in reversed(ip_incs):  # Most recent first
            record = mgr._incidents.get(inc_id)
            if record and record.is_active:
                return inc_id
        # Fallback: search all incidents
        for inc_id, record in mgr._incidents.items():
            if record.ip == ip and record.is_active:
                return inc_id
        return None
...
class DiscordBot:
    ...
    def send_incident_alert(self, incident_dict):
        client = self._get_client()
        if not client:
            logger.warning("Discord not configured; incident alerts disabled")
            return False
        try:
            return client.send_incident_alert(incident_dict)
        except Exception as e:
            logger.warning("Discord send_incident_alert error: %s", e)
            return False
        ...
        class OPNsenseBot(discord.Client):
            ...
            async def _register_slash_commands(self):
                ...
                commands = [
                    ...
                    {
                        'name': 'incident',
                        'description': 'View or transition an incident by ID or IP address',
                        'options': [
                            {
                                'type': 3,  # STRING
                                'name': 'incident_id',
                                'description': 'Incident ID (e.g. inc_abc123) or IP address (e.g. 192.168.1.1)',
                                'required': True,
                            },
                            {
                                'type': 3,  # STRING
