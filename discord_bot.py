...
class DiscordClient:
    ...
    def send_incident_alert(self, incident_dict: Dict[str, Any]) -> bool:
        """Send an incident alert with narrative to Discord.

        Uses the incident's narrative field (generated by correlation_engine.Incident.get_narrative())
        as the embed description, and adds structured fields for signal count, sources,
        attack chain, and severity.

        Args:
            incident_dict: Incident.to_dict() output with 'narrative', 'severity', 'ip', etc.
        ...
        # Dedup key: incident alerts per IP within 5 min
        signal = 'INCIDENT'
        dedup_key = f"INCIDENT:{ip}"
        if not self.rate_limiter.should_alert(signal, dedup_key):
            return False

        # Title — use escalation prefix when applicable
        if is_escalated:
            title = f"ESCALATED INCIDENT: {ip}"
        elif len(phases) >= 2:
            title = f"ATTACK CHAIN DETECTED: {ip}"
        else:
            title = f"INCIDENT: {ip}"
        ...
        fields: List[Dict[str, Any]] = [
            ...
            {'name': 'Sources', 'value': ', '.join(sources) or 'N/A', 'inline': True},
        ]

        if phases:
            fields.append({'name': 'Attack Chain', 'value': ' → '.join(phases), 'inline': False})

        if signal_types:
            fields.append({'name': 'Signal Types', 'value': ', '.join(signal_types[:8]), 'inline': False})

        # Narrative as description (truncated to Discord's 4096-char limit)
        description = narrative[:4000] if narrative else 'Incident detected.'
        ...
        result = self._post(f'channels/{self.channel_id}/messages', payload)
        if result:
            logger.info("Discord incident alert sent for %s [%s]", ip, severity.upper())
        return result
