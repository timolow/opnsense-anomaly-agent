...
export const api = {
  ...
  events: async (limit = 100, offset = 0): Promise<EventsData> => {
    ...
    const events = raw.map((e: any) => ({
      ...
      protocol: 'ip',
      src_ip: e.ip || '0.0.0.0',
      src_hostname: (e as any).src_hostname || null,
      dst_ip: '',
      src_port: 0,
      dst_port: 0,
  ...
  alerts: async (): Promise<AlertsData> => {
      ...
      rawAlerts.forEach((a: any) => {
        anomalies.push({
          ...
          src_hostname: (a as any).src_hostname || null,
          destination_ip: '',
      ...
      rawAnomalies.forEach((a: any) => {
        anomalies.push({
          ...
          dst_hostname: (a as any).dst_hostname || null,
  ...
  serviceStatus: async (): Promise<ServiceStatusData> => {
    ...
    return {
      ...
      alerts: [],
      unbound: {
        status: (unboundSvc.monitored === true || (unboundSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        cache_size: (unboundSvc.metrics?.cache_size as number) || 0,
        queries_total: (unboundSvc.total_events as number) || 0,
        queries_cached: 0,
        status_icon: (unboundSvc.monitored === true || (unboundSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        details: '',
      },
      dhcp: {
        status: (dhcpSvc.monitored === true || (dhcpSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        leases: (dhcpSvc.metrics?.leases_24h as number) || 0,
        active_leases: (dhcpSvc.metrics?.leases_24h as number) || 0,
        status_icon: (dhcpSvc.monitored === true || (dhcpSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        details: '',
      },
      ntp: {
        status: (ntpSvc.monitored === true || (ntpSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        server: 'default',
        offset: 0,
        status_icon: (ntpSvc.monitored === true || (ntpSvc.total_events || 0) > 0) ? 'running' : 'stopped',
        details: '',
      },
    };
  },
